...
public class ReaderInfoDao {
    ...
    private final static String UPDATE_READER_INFO="UPDATE reader_info set name = ? ,sex = ? ,birth = ? ,address = ? ,telcode = ?, permission = ?, departmentID = ?  where reader_id = ? ";
        ...
        jdbcTemplate.query(ALL_READER_INFO_SQL, new RowCallbackHandler() {
            public void processRow(ResultSet resultSet) throws SQLException {
                ...
                while (resultSet.next()){
                    ...
                    reader.setDepartmentID(resultSet.getInt("departmentID"));
                    reader.setPermission(resultSet.getInt("permission"));
                    readers.add(reader);
                }
            }
        });
    ...
    public int editReaderInfo(ReaderInfo readerInfo){
        ...
        int departmentID = readerInfo.getDepartmentID();
        int permission = readerInfo.getPermission();
        return jdbcTemplate.update(UPDATE_READER_INFO,new Object[]{name,sex,birth,address,telcode,permission,departmentID,readerId});
    }
    ...
    public int addReaderInfo(ReaderInfo readerInfo){
        ...
        return jdbcTemplate.update(ADD_READER_INFO_SQL,new Object[]{readerId,name,sex,birth,address,telcode,permission,departmentID});
    }
